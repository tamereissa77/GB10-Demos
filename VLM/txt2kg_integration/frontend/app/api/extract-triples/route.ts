        ...
        const messages = [
          {
            ...
            content: `You are a knowledge graph builder that extracts structured information from text in ANY language, including Arabic.

Your task: Extract subject-predicate-object triples from the given text and return them as a JSON array.

Rules:
- Extract entities (people, organizations, dates, amounts, roles, locations) and their relationships
- Output ALL values in English (transliterate Arabic names, translate Arabic terms)
- Return ONLY a valid JSON array, no other text
- Each triple must have exactly three fields: "subject", "predicate", "object"

Example output format:
[
  {"subject": "National Bank of Egypt", "predicate": "is_a", "object": "Egyptian joint-stock company"},
  {"subject": "Ahmed Hassan", "predicate": "role", "object": "Chairman of the Board"}
]`
          },
          {
            role: 'user' as const,
            content: `Extract all knowledge triples from this text. Return ONLY a JSON array:\n\n${text}`
          }
        ];
        ...
        console.log(`[${new Date().toISOString()}] extract-triples: Raw response preview: ${response?.substring(0, 500)}`);
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            // Fallback parser
            console.warn('No JSON array found in response, trying fallback parser');
            triples = parseTriplesFallback(response);
          }
        } catch (parseError) {
          console.warn('Failed to parse JSON response, using fallback parser:', parseError);
          console.warn('Response that failed parsing:', response?.substring(0, 1000));
          triples = parseTriplesFallback(response);
        }
