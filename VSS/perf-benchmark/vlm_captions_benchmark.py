...
"""
VLM Captions Benchmark Implementation

Tests concurrent VLM caption generation at different concurrency levels.
"""
...
class VlmCaptionsBenchmark(BenchmarkBase):
    """VLM captions benchmark - test concurrent VLM caption generation"""
    ...
    def parse_benchmark_config(self, scenario_config: Dict, global_config: Dict) -> Dict[str, Any]:
        """Parse VLM captions benchmark configuration"""
        if scenario_config.get("benchmark_mode") != "vlm_captions_burst":
        ...
        vlm_captions_params = self._merge_with_defaults(
            scenario_config.get("vlm_captions_params", {}),
            global_config.get("vlm_captions_params", {}),
        )

        return {
            "videos": scenario_config["videos"],
            "vlm_captions_params": vlm_captions_params,
    ...
    def execute(self, config: Dict, scenario_name: str) -> Dict[str, Any]:
        """Execute VLM captions benchmark"""
        self.logger.info(f"Starting VLM captions benchmark: {scenario_name}")
        ...
        self.logger.info(
            f"VLM captions benchmark completed: {execution_results['successful_test_cases']}/"
    ...
    def _generate_test_case_id(self, video_config: Dict, chunk_size: int) -> str:
        ...
        return f"vlm_captions_{name}_{chunk_size}sec"
    ...
    def _execute_vlm_captions_test_case(
        ...
        """Execute VLM captions test case for all concurrency levels"""
        self.logger.info(
            f"Starting VLM captions test: {test_case_id} with levels {video_config['concurrency_levels']}"
        )
        ...
        results_file = os.path.join(test_case_dir, "vlm_captions_results.json")
    ...
    def _process_single_vlm_caption(
        ...
        try:
            ...
            # Get VLM captions params
            params = self._merge_with_defaults(
                video_config.get("vlm_captions_params", {}), benchmark_config["vlm_captions_params"]
            )
            ...
            if "enable_reasoning" in params:
                request_data["enable_reasoning"] = params["enable_reasoning"]

            # Add optional CV metadata parameters if user configured them
            ...
            # Add VLM prompts
            video_vlm_prompts = video_config.get("vlm_prompts", {})
            global_vlm_prompts = benchmark_config.get("vlm_prompts", {})

            if video_vlm_prompts.get("prompt") or global_vlm_prompts.get("prompt"):
                request_data["prompt"] = video_vlm_prompts.get(
                    "prompt", global_vlm_prompts.get("prompt")
                )

            if video_vlm_prompts.get("system_prompt") or global_vlm_prompts.get("system_prompt"):
                request_data["system_prompt"] = video_vlm_prompts.get(
                    "system_prompt", global_vlm_prompts.get("system_prompt")
                )

            self.logger.debug(
                f"VLM captions request payload:\n{json.dumps(request_data, indent=2)}"
            )

            vlm_response = self.make_api_call(
                "/generate_vlm_captions", method="POST", data=request_data
            )
            processing_time = time.time() - start_time
            self.latency_tracker.record_latency(processing_time, f"file_{file_index}")

            self.save_response(
                vlm_response, test_case_dir, f"vlm_captions_response_file_{file_index}.json"
            )
    ...
    def analyze_results(self, results_dir: str, output_file: str) -> None:
        """Generate Excel report from VLM captions benchmark results"""
        self.logger.debug(f"Analyzing VLM captions results from: {results_dir}")
        ...
        for test_case in execution_summary["test_cases"]:
            ...
            if os.path.exists(results_file):
                ...
                for concurrency_result in vlm_results.get("concurrency_results", []):
                    ...
                    level_result = {
                        ...
                        "benchmark_mode": "vlm_captions_burst",
                        "video_file": os.path.basename(vlm_results.get("video_file", "")),
                        "chunk_size": vlm_results.get("chunk_size", 0),
                        "concurrency_level": concurrency_level,
        ...
        try:
            self.logger.debug("Adding plots to VLM captions Excel report...")
            if all_concurrency_results:
        ...
        self.logger.debug(f"VLM captions results analysis completed: {output_file}")
