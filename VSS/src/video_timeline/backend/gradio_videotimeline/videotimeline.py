...
class VideoTimeline(StreamingOutput, Component):
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: path or URL for the default value that VideoTimeline component is going to take. Can also be a tuple consisting of (video filepath, subtitle filepath). If a subtitle file is provided, it should be of type .srt or .vtt. Or can be callable, in which case the function will be called whenever the app loads to set the initial value of the component.
            ...
            timestamps: list of timestamps for the video.
            marker_labels: list of marker labels for the video.
            start_times: list of start times for the video.
            end_times: list of end times for the video.
            descriptions: list of descriptions for the video.
    ...
    def preprocess(self, payload: VideoData | None) -> str | None:
        ...
        if self.min_length is not None or self.max_length is not None:
            ...
            if self.min_length is not None and duration < self.min_length:
                raise gr.Error(
                    f"VideoTimeline is too short, and must be at least {self.min_length} seconds"
                )
            if self.max_length is not None and duration > self.max_length:
                raise gr.Error(
                    f"VideoTimeline is too long, and must be at most {self.max_length} seconds"
                )
    ...
    def postprocess(
        self, value: str | Path | tuple[str | Path, str | Path | None] | dict | None
    ) -> VideoData | None:
        """
        Parameters:
        ...
        """
        logging.debug(f"========== Value before `postprocess` function: ==========\n{value}")
        if self.streaming:
        ...
        if isinstance(value, (str, Path)):
            ...
            video = value.get('video', None)
            subtitles = value.get('subtitles', None)
            processed_files = (
                self._format_video(video),
                self._format_subtitle(subtitles),
            )
            timestamps = value.get('timestamps', None)
            marker_labels = value.get('marker_labels', None)
            start_times = value.get('start_times', None)
            end_times = value.get('end_times', None)
            descriptions = value.get('descriptions', None)

            assert timestamps, 'Missing key timestamps in VideoEvent dict input'
            assert marker_labels, 'Missing key marker_labels in VideoEvent dict input'
            assert start_times, 'Missing key start_times in VideoEvent dict input'
            assert end_times, 'Missing key end_times in VideoEvent dict input'

        else:
            raise Exception(f"Cannot process type as video: {type(value)}")
        if not processed_files[0]:
            raise ValueError("VideoTimeline data missing")
        return VideoData(video=processed_files[0],
                         subtitles=processed_files[1],
    ...
    def _format_video(self, video: str | Path | None) -> FileData | None:
        ...
        if (
            ...
            warnings.warn(
                "VideoTimeline does not have browser-compatible container or codec. Converting to mp4."
            )
    ...
    async def stream_output(
        ...
        if not value.endswith(".ts"):
            if not value.endswith(".mp4"):
                raise RuntimeError(
                    "VideoTimeline must be in .mp4 or .ts format to be streamed as chunks",
                )
